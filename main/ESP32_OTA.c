...
static void main_application_task(void *pvParameters)
{
        ...
        while (1)
        {
                ...
                esp_mqtt_client_publish(mqtt_client, TB_TELEMETRY_TOPIC, post_data, 0, 0, 0);
                cJSON_Delete(root);
                // Free is intentional, it's client responsibility to free the result of cJSON_Print
                free(post_data);
        ...
        while (1)
        {
                ...
                case STATE_WAIT_MQTT:
                {
                        ...
                        if (actual_event & (WIFI_CONNECTED_EVENT | MQTT_CONNECTED_EVENT))
                        {
                                ...
                                esp_mqtt_client_publish(mqtt_client, TB_ATTRIBUTES_TOPIC, current_fw_attribute, 0, 0, 0);
                                printf("HELLO222222");
                                // Free is intentional, it's client responsibility to free the result of cJSON_Print
                                ...
                                esp_mqtt_client_publish(mqtt_client, TB_ATTRIBUTES_REQUEST_TOPIC, TB_SHARED_ATTR_KEYS_REQUEST, 0, 0, 0);
...
void app_main()
{
        ...
        xTaskCreate(&main_application_task, "main_application_task", 10240, NULL, 6, NULL);
        xTaskCreate(&test_version, "test_version", 1024, NULL, 4, NULL);
}
